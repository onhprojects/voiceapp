...
This agent is complemented by specialized instruction files in `.github/agents/instructions/`:

- **database-schema.instructions.md** — Applied to SQL migration files (`**/migrations/**/*.sql`). Ensures schema design follows Supabase Postgres best practices, includes RLS policies, proper indexing, and security guidelines.
- **server-supabase.instructions.md** — Applied to server actions and API routes (`**/actions.ts`, `**/app/api/**/*.ts`). Ensures proper auth validation, type safety, error handling, and Next.js best practices.
- **client-supabase.instructions.md** — Applied to client components (`**/components/**/*client*.tsx`). Ensures proper auth handling, uses anon key only, implements real-time subscriptions safely, and follows React best practices.

These instructions automatically apply to matching files in your project, providing context-specific guidance without additional configuration.
