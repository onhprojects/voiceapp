...
export function PublicIntakeForm() {
  ...
  const startSession = useCallback(async () => {
    if (!sessionId) {
      setError('Session ID not generated')
      return
    }

    try {
      const assessment = await createPublicAssessment({
        sessionId,
  ...
  const handleRecordingComplete = useCallback(
    async (
      ...
      if (!assessmentId) {
        setError('Assessment not initialized')
        return
      }
      ...
      try {
        // Upload audio file
        const filePath = await uploadPublicAudioFile(sessionId, questionNumber, blob)

        // Save response record
        await savePublicIntakeResponse(assessmentId, {
          assessmentId,
          questionNumber,
          questionText,
          sectionTitle,
  ...
  const exportAssessment = async () => {
    if (!assessmentId) {
      setError('No assessment to export.')
      return
    }
    ...
    try {
      alert(
        `Assessment submitted! Recorded ${recordedCount} out of ${totalQuestions} questions (${Math.round((recordedCount / totalQuestions) * 100)}%)\n\nSession ID: ${sessionId}`
      )
      setError(null)
    } catch (err) {
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Record responses to all questions. Your audio will be securely saved.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <CardContent>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <Button onClick={exportAssessment} disabled={isExporting} className="w-full sm:w-auto">
              ...
              {isExporting ? 'Submitting...' : 'Submit Assessment'}
            </Button>
          </div>
            ...
            <div>
              <p className="text-sm text-gray-600">
                ...
                <span className="font-medium text-gray-900">
                  {recordedCount > 0 ? 'In Progress' : 'Not Started'}
                </span>
              </p>
              <p className="text-sm text-gray-600 mt-1">
                Completion:{' '}
                <span className="font-medium text-gray-900">
                  {Math.round((recordedCount / totalQuestions) * 100)}%
                </span>
              </p>
